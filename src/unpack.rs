...
#[fehler::throws]
pub(crate) fn unpack(version: &crates_index::Version, mut archive: tar::Archive<impl std::io::Read>, output: impl AsRef<Path>) {
    ...
    while let Some(mut entry) = entries.next().transpose()? {
        ...
        if path.components().any(|c| matches!(c, Component::ParentDir | Component::RootDir | Component::Prefix(_))) {
            fehler::throw!(anyhow!("a file in the archive ({}) contains a .. or root segment", path.display()));
        }
